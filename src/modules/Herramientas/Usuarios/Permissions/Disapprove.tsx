...
const DisapproveUsuarios = ({ setShowDisapprove, selected, reload }) => {
    ...
    const onclick = async () => {
        ...
        try {
            ...
            if (selected.estado === "INACTIVO")
                return sendMessage("El usuario ya está INACTIVO", "Error");
            const response = await axios.patch(`/herramientas/patchUser/${idSelected}`, { estado: "INACTIVO" });
            if (!response) throw new Error("Error en la desactivación del usuario");
...
export default DisapproveUsuarios;
