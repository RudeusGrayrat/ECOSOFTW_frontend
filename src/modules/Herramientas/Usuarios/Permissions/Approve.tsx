...
const ApproveUsuarios = ({ selected, setShowApprove, reload }) => {
    ...
    const aprobar = async () => {
        ...
        try {
            ...
            if (selected.estado === "ACTIVO")
                return sendMessage("El usuario ya está ACTIVO", "Error");
            const response = await axios.patch(`/herramientas/patchUser/${idSelected}`, { estado: "ACTIVO" });
            if (!response) throw new Error("Error en la activación del usuario");
...
export default ApproveUsuarios;
