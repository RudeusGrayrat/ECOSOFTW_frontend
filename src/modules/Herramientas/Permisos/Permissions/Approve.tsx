...
const ApprovePermisos = ({ selected, setShowApprove, reload }) => {
    ...
    const aprobar = async () => {
        ...
        try {
            ...
            if (selected.estado === "ACTIVO")
                return sendMessage("El permiso ya está ACTIVO", "Error");
            const response = await axios.patch(`/herramientas/patchPermission/${idSelected}`, { estado: "ACTIVO" });
            if (!response) throw new Error("Error en la activación del permiso");
